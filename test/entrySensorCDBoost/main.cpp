...
#include "../../atomics/entrySensor.hpp" //ADD FILENAME
...
int main(int argc, char ** argv) {
  ...
  auto EntrySensor1 = make_atomic_ptr<entrySensor<Time,Message>>();

  shared_ptr<flattened_coupled<Time, Message>> TOP(new flattened_coupled<Time, Message>(
  {input_test_generator, EntrySensor1}, 
  {}, 
  {{input_test_generator, EntrySensor1}}, 
  {EntrySensor1}
  ));  
  ...
  //CHANGE ACCORDING TO FILE
    ...
    ofstream out_data("entrySensor_test_output.txt"); //CHANGE  
