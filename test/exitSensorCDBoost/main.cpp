...
#include "../../atomics/exitSensor.hpp" //ADD FILENAME
...
int main(int argc, char ** argv) {
  ...
  auto ExitSensor1 = make_atomic_ptr<exitSensor<Time,Message>>();

  shared_ptr<flattened_coupled<Time, Message>> TOP(new flattened_coupled<Time, Message>(
  {input_test_generator, ExitSensor1}, 
  {}, 
  {{input_test_generator, ExitSensor1}}, 
  {ExitSensor1}
  ));  
  ...
  //CHANGE ACCORDING TO FILE
    ...
    ofstream out_data("exitSensor_test_output.txt"); //CHANGE  
